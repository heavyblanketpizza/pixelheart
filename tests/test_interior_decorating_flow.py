...
class InteriorDecoratingFlowTests(unittest.TestCase):
    ...
    def test_choose_rotate_place_and_repeat_keeps_held_item_independent(self):
        dialog = self.editor()
        self.choose_chair(dialog)
        self.move(dialog, 6, 7)
        ...
        self.click_tile(dialog, 9, 8, Qt.MouseButton.RightButton)
    ...
    def test_stop_placing_drag_put_away_and_undo_restores_the_piece(self):
        dialog = self.editor()
        self.choose_chair(dialog)
        self.click_tile(dialog, 6, 7)
        QTest.keyClick(dialog.canvas, Qt.Key.Key_Escape)
